...
const App: React.FC = () => {
  ...
  const [isFacilitator] = useState(false);
  const [activeFacilitator] = useState<{ name: string; subject: string } | null>(null);
  const [activePupil] = useState<ProcessedStudent | null>(null);
  ...
  // --- SUPABASE PERSISTENCE HANDSHAKE ---
  useEffect(() => {
    const fetchInstitutionalData = async () => {
      setShowSyncOverlay(true);
      try {
        const { data: remoteSettings } = await supabase.from('app_settings').select('payload').single();
        if (remoteSettings?.payload) setSettings(remoteSettings.payload);

        const { data: remoteStudents } = await supabase.from('students').select('payload');
        if (remoteStudents?.length) setStudents(remoteStudents.map(s => s.payload));

        const { data: remoteFacilitators } = await supabase.from('facilitators').select('payload');
        if (remoteFacilitators?.length) {
            const facMap: Record<string, StaffAssignment> = {};
            remoteFacilitators.forEach(f => { facMap[f.payload.taughtSubject || 'unknown'] = f.payload; });
            setFacilitators(facMap);
        }
        setIsHostConnected(true);
      } catch (e) {
        console.warn("Supabase Handshake failed, operating on local cache.", e);
      } finally {
  ...
  const handleSave = useCallback(async () => {
    ...
    setShowSyncOverlay(true);
    try {
      // Syncing institutional settings
      await supabase.from('app_settings').upsert({ id: 1, payload: settings });
      
      // Batch syncing pupils (simplified)
      for (const student of students) {
          await supabase.from('students').upsert({ id: student.id, payload: student });
      }

      console.log("Institutional Data Sync successful.");
    } catch (e) {
      console.error("Critical Sync Error:", e);
    } finally {
      setShowSyncOverlay(false);
    }
  }, [settings, students, isRemoteViewing]);
    ...
    <div className={`min-h-screen bg-gray-100 font-sans flex flex-col ${isLandscapeView ? 'print-landscape' : 'print-portrait'}`}>
      ...
      {showSyncOverlay && (
        <div className="fixed inset-0 z-[1000] bg-blue-900/80 backdrop-blur-md flex flex-col items-center justify-center text-white space-y-4">
           ...
           <p className="text-xl font-black uppercase tracking-[0.4em] animate-pulse">Syncing Academy Node...</p>
        </div>
      )}
      ...
      <div className="no-print bg-blue-950 text-white p-3 sticky top-0 z-50 shadow-2xl flex flex-wrap justify-between items-center gap-4">
        <div className="flex flex-wrap items-center gap-6">
          ...
          <div className="flex flex-col gap-1">
             <div className="flex items-center justify-between px-1">
                <span className="text-[8px] font-black text-blue-400 uppercase tracking-widest">United Baylor Academy Hub</span>
                <div className="flex items-center gap-1">
                   <div className={`w-1.5 h-1.5 rounded-full ${isHostConnected ? 'bg-green-500 animate-pulse' : 'bg-slate-600'}`}></div>
                   <span className="text-[7px] font-black uppercase text-slate-500">{isHostConnected ? 'Hub Linked' : 'Disconnected'}</span>
                </div>
             </div>
        ...
        <div className="flex gap-2">
           {!isRemoteViewing && <button onClick={handleSave} className="bg-yellow-500 hover:bg-yellow-600 text-blue-950 px-5 py-2 rounded-xl font-black shadow-xl transition-all active:scale-95 text-[10px] uppercase tracking-widest">Sync Hub Records</button>}
