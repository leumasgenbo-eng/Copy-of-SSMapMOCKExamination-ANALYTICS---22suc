    ...
    <div className="bg-white p-4 print:p-0 min-h-screen max-w-[420mm] mx-auto overflow-hidden print:overflow-visible print:max-w-none">
      ...
      <div className="no-print mb-8 space-y-4">
        <div className="flex items-center justify-between bg-white p-4 rounded-3xl border border-gray-100 shadow-sm">
          <div className="flex items-center gap-4">
            <h3 className="text-xs font-black text-gray-500 uppercase tracking-widest border-r pr-4 border-gray-200">Controller</h3>
          ...
          <div className="flex gap-3">
             <button onClick={handleShareStats} className="bg-green-600 hover:bg-green-700 text-white px-5 py-2.5 rounded-xl font-black text-[10px] uppercase shadow-lg transition-all active:scale-95">Share</button>
             <button onClick={handleDownloadPDF} disabled={isGenerating} className="bg-red-700 hover:bg-black text-white px-5 py-2.5 rounded-xl font-black text-[10px] uppercase shadow-lg transition-all active:scale-95">PDF</button>
             <button onClick={() => window.print()} className="bg-blue-950 hover:bg-black text-white px-5 py-2.5 rounded-xl font-black text-[10px] uppercase shadow-lg transition-all active:scale-95">Print</button>
          </div>
        </div>
        <div className="grid grid-cols-10 gap-1 p-1 bg-gray-100 rounded-xl border border-gray-200">
           {MOCK_SERIES.map(mock => (
             <button key={mock} onClick={() => onSettingChange('activeMock', mock)} className={`py-2 rounded-lg text-[9px] font-black uppercase transition-all ${settings.activeMock === mock ? 'bg-blue-900 text-white' : 'bg-white text-blue-900'}`}>{mock.split(' ')[1]}</button>
           ))}
        </div>
      </div>
      ...
      <div id="broadsheet-export-container">
        <div className="text-center mb-10 relative">
          ...
          <div className="flex items-center justify-center gap-6 text-[11px] font-black text-gray-800 uppercase tracking-widest">
        ...
        <div className="min-h-[400px]">
          ...
          {sheetView === 'analytics' && <InstitutionalAnalytics students={students} stats={stats} settings={settings} facilitators={facilitators} />}
        </div>
        ...
        <div className="flex justify-between items-end pt-12 pb-4 border-t-2 border-blue-900 mt-12 page-break-inside-avoid">
           ...
           <div className="w-48 border-t-2 border-gray-900 text-center font-black uppercase text-[10px] pt-2">Director Approval</div>
        </div>
      </div>
    </div>
  );
