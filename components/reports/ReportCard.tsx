...
const ReportCard: React.FC<ReportCardProps> = ({ student, stats, settings, onSettingChange, onStudentUpdate, classAverageAggregate, totalEnrolled, isFacilitator }) => {
  ...
  const dynamicAnalysis = useMemo(() => {
    ...
    const strengthText = strengths.length > 0 ? `Exhibits strong mastery in ${strengths.slice(0, 2).join(", ")}, performing significantly above the ${genderTerm} benchmark.` : `Maintains a steady academic profile within ${regionalLocality}.`;
    const weaknessText = weaknesses.length > 0 ? `Remedial intervention in ${weaknesses.slice(0, 1).map(w => w.name).join("")} is advised to match the cohort average of ${weaknesses[0].mean}%.` : `Academic output is highly competitive.`;
    return { performance: `${strengthText} ${weaknessText}`, recommendation: student.bestSixAggregate <= 15 ? "Outstanding result. Maintain momentum." : "Focus on theoretical applications." };
  ...
  const handleSharePDF = async () => {
    ...
    const opt = { margin: 0, filename: `${student.name.replace(/\s+/g, '_')}_Report.pdf`, image: { type: 'jpeg', quality: 0.98 }, html2canvas: { scale: 2 }, jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' } };
    try {
        // @ts-ignore
  ...
  return (
    <div id={`report-${student.id}`} className="bg-white p-10 max-w-[210mm] mx-auto min-h-[296mm] border border-gray-200 shadow-2xl print:shadow-none print:border-none page-break relative flex flex-col box-border font-sans">
       ...
       <div data-html2canvas-ignore="true" className="absolute top-4 -right-16 flex flex-col gap-4 no-print z-50">
          <button onClick={handleSharePDF} disabled={isGenerating} className={`${isGenerating ? 'bg-gray-400' : 'bg-blue-900'} text-white w-12 h-12 rounded-full shadow-xl flex items-center justify-center transform hover:scale-110`}>
       ...
       <div className="relative border-b-[6px] border-double border-blue-900 pb-4 mb-6 text-center">
          ...
          <div className="mb-3">
             <span className="text-[9px] font-black text-blue-900 border border-blue-900/20 px-5 py-0.5 rounded-full uppercase tracking-[0.4em] bg-blue-50/50">
               HUB ID: <EditableField value={settings.schoolNumber} onChange={(v) => onSettingChange('schoolNumber', v)} className="inline-block" />
             </span>
          </div>
          ...
          <div className="flex justify-center gap-8 text-[11px] font-black text-gray-800 uppercase tracking-widest">
             ...
             <span className="border-x border-gray-300 px-6 italic">AY: <EditableField value={settings.academicYear} onChange={(v) => onSettingChange('academicYear', v)} /></span>
       ...
       <div className="grid grid-cols-2 gap-6 mb-6 border-2 border-blue-950 p-4 rounded-3xl bg-blue-50/10 text-[12px] font-bold">
          ...
          <div className="space-y-2 pl-2 text-right">
            <div className="flex items-center justify-between"><span className="text-gray-400 uppercase text-[10px]">Aggregate:</span><span className="text-3xl font-black text-blue-950">{student.bestSixAggregate}</span></div>
            <div className="flex items-center justify-between"><span className="text-gray-400 uppercase text-[10px]">Rank:</span><span className="font-black text-blue-900">{student.rank} / {totalEnrolled}</span></div>
          </div>
       </div>
            ...
            <tbody>
               {student.subjects.map(sub => (
                 <tr key={sub.subject} className="even:bg-gray-50/50 border-b border-gray-100 font-bold">
                   <td className="px-4 py-2.5 text-blue-950 uppercase">{sub.subject}</td>
                   ...
                   <td className="px-3 py-2.5 text-[9px] font-black text-blue-800 uppercase italic opacity-60 truncate max-w-[120px]">{sub.facilitator}</td>
                 </tr>
               ))}
            </tbody>
         </table>
       ...
       <div className="mt-auto pt-4 border-t border-gray-200">
          <div className="bg-gray-900 text-white p-4 rounded-xl space-y-2">
             ...
             <p className="text-[7px] leading-relaxed opacity-70">Grades are calculated relative to the cohort mean (μ) and standard deviation (σ). Lower aggregate = Higher distinction.</p>
          </div>
       </div>

       <div className="flex justify-between items-end mt-auto pb-6 pt-10 border-t border-gray-100">
         <div className="w-[30%] text-center border-t-2 border-gray-900 pt-2 text-[9px] font-black uppercase text-gray-500">Academy Director</div>
         <div className="w-[30%] text-center border-t-2 border-gray-900 pt-2">
            ...
            <p className="text-[10px] font-black text-red-700 font-mono uppercase">{new Date(settings.nextTermBegin).toLocaleDateString(undefined, { dateStyle: 'long' })}</p>
         </div>
       </div>
    </div>
  );
